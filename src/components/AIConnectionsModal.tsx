...
interface AIConnectionsModalProps {
  ...
  onSave: (connection: CustomAIConnection) => void;
                ...
                {connections.map((connection) => (
                  <div key={connection.id} className="py-3 flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-500 truncate">{serviceDetails(connection.service).label} · {connection.model} · {maskedKey(connection.apiKey)}</p>
                    ...
                    <button type="button" onClick={() => { if (window.confirm(`Delete “${connection.name}”?`)) onDelete(connection.id); }} className="p-1.5 text-slate-500 hover:text-red-700 hover:bg-red-50 rounded-md" aria-label={`Delete ${connection.name}`}><Trash2 size={15} /></button>
                  </div>
                ))}
              </div>
            </section>
            ...
            <form onSubmit={saveDraft} className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="sm:col-span-2 flex items-center justify-between gap-3 pt-1">
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  <button type="submit" disabled={!canSave} className="px-3 py-2 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-md disabled:opacity-40">Save connection</button>
                </div>
              </div>
            </form>
          </section>
