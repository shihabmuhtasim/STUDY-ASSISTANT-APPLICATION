...
export async function POST(request: Request) {
  ...
  try {
    const result = await routeAIRequest({ prompt, pageNumber: body.pageNumber, pageText: body.pageText?.slice(0, 12_000), documentText: body.documentText?.slice(0, 48_000), pageImage: body.pageImage, history: body.history?.slice(-3), modelPreference: body.modelPreference, allowFallback: body.allowFallback });
